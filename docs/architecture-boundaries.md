...
- `maxStoredChatMessages` 和 `maxCanonicalTurns` 限制单会话可恢复历史长度。
- `maxConversations`、`maxConversationsPerTenant`、`maxConversationApproxBytes`、`maxTotalApproxBytes` 是主动驱逐预算，默认关闭，部署方可按机器内存开启。
- 驱逐必须同时清理 `response_id -> conversationKey` 索引，不能留下悬挂 continuation。
- diagnostics 只暴露计数、近似字节、截断标记和驱逐计数，不能暴露原始 prompt、tool 参数或 response body。
...
- `tests/protocol-diagnostics.test.js`
