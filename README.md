...
当前推荐单实例部署。Relay 会在进程内维护短期 conversation state，用于补全 Responses/WS 增量请求再转换到 Chat Completions 或 Anthropic；单实例可以保证这些状态稳定命中。默认状态保留 24 小时，可通过 `RELAY_CONVERSATION_STATE_TTL_MS` 调整。
