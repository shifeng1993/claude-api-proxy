...
- ✅ **多模式支持** - 自由选择 OpenAI 兼容 API 或 GitHub Copilot
...
代理会在每个请求的 system message 末尾自动注入行为规则，引导模型使用中文思考、合理规划任务等。规则统一维护在 `src/config/system-prompts.js` 中，可按需增删。

当前注入的规则：
- 始终使用中文进行思考和推理
- 中等以上复杂度任务优先使用 skill / plan mode / 子 Agent
- 不懂的内容不猜测，暂停询问用户
